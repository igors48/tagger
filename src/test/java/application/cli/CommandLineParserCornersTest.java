...
public class CommandLineParserCornersTest {
    ...
    @Test
    public void whenMoreThaTwoArgumentsThenErrorThrown() {
        try {
            CommandLineParser.parse(new String[]{"1", "2", "3"});
            fail();
        } catch (ApplicationError applicationError) {
    ...
    public void whenOneArgumentsThenItIsReturnedInParameters() throws ApplicationError {
        assertEquals("1", CommandLineParser.parse(new String[]{"1"}).getMusicPath());
    }
